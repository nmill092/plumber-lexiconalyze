...
<Modal on:modal-close={() => (showWordcloudModal = false)}>
    <span slot="modal-header">Wordcloud <a class="text-sm bg-rose-500 p-2 text-white rounded-lg" target="_blank" rel="noreferrer" on:click={() => window.open(wordcloudSrc, "_blank")} href="#">Open in separate window</a></span>
    <span slot="modal-body">
        {#if wordcloudSrc}
        <img src={wordcloudSrc} alt="Sentiment wordcloud"/>
        {:else}
